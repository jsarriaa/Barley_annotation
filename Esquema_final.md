...
Prepare reference chunks for Helixer/partition-based steps:
bash scripts/run_split_genome.sh
1) Mandatory TE annotation (EDTA)
Activate EDTA environment (EDTA 2.2.2).
Run EDTA pipeline script:
nohup bash scripts/run_edta.sh > logs/EDTA/run_edta.log 2>&1 &
Convert TE output to hints:
bash scripts/run_merge_EDTA.sh
python3 scripts/run_EDTA2hints.py
A) Build evidence
Run RNA-seq mapping and transcript assembly:
STAR index and mapping
BAM merge
Portcullis filtering
StringTie assembly and merge
Run IsoSeq mapping with minimap2 and convert to hints.
Run miniprot/miniprothint protein evidence and generate hints:
...
Build RNA hints and merge all evidence hints (RNA + IsoSeq + TE + protein).
B) Build gene predictions
Run AUGUSTUS with merged hints and extract supported models.
Run TSEBRA selection.
Run Helixer and combine chromosome outputs.
Convert inputs to EVM format, apply weights, run EVM, and keep cleaned EVM output.
C) Final structural refinement
Run Mikado with SwissProt plants database:
final retained file:
...
Keep the PASA-updated GFF3 as final annotation:
...
Prepare COCLA workspace and databases:
...
Keep the generated COCLA output folder for wrap-up:
